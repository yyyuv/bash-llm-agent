...
**Remaining Phase 3 gate item:** the live three-model comparison (`openai/gpt-4o-mini` native, `ollama/mistral:7b` native, `ollama/llama3:8b` prompted) is not yet run — **Ollama is not installed on this machine** (`which ollama` → not found). The user must install Ollama + `ollama pull mistral:7b llama3:8b`, then run cases 1–8 of [tests/cases.md](tests/cases.md) per model and curate divergent transcripts into `logs/phase3/`.

Notes: deps installed via `pip3 install --user litellm` on system Python 3.9; `OPENAI_API_KEY` lives in a gitignored `.env` at the repo root (`set -a; source .env; set +a` before running — plain `source` does not export it to the doit subprocess); `~/.local/bin` must be on PATH (added to `~/.zshrc`). Local Ollama models need no API key. **Next: finish the Phase 3 live model comparison (needs Ollama), then the joint review before Phase 4 (⏸ scope gate — decisions locked only through Phase 3).**
