...
**Next:** Phase 7 (user shell-history awareness — the `PROMPT_COMMAND`/`precmd` hook, `~/.doit/shell_hist/`, distinguishing user-run from doit-run commands) can now build on a working `DOIT_SESSION`/shell-wrapper foundation. **Phases 7–9 remain ⏸-gated** — re-review scope before starting Phase 7 proper.
