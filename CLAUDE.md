...
**Next: capture a live two-turn transcript** (e.g. "list the files here" → "now sort them by date") on `openai/gpt-4o-mini` to prove follow-up resolution end-to-end, curate into `logs/phase4/`. Run: `set -a; source .env; set +a` then `DOIT_SESSION=demo doit "..."` twice with the same `DOIT_SESSION`. **Then Phases 5–9 remain gated** (⏸ scope) — re-review each before starting.
