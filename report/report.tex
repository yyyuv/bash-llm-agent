...
Rather than defining tools for specific commands (e.g., \texttt{ls} or \texttt{grep}), our agent uses a unified set of five universal tools: \texttt{run\_command}, \texttt{ask\_user}, \texttt{answer}, \texttt{remember}, and \texttt{change\_dir}. We structure this report to follow the exact sections defined in the assignment specifications, detailing the design decisions, implementation details, log evidence, and limitations encountered for each module.

\subsection{Context Assembly and Code Mapping}
\label{sec:context_mapping}
Every block of context defined in our formal Agent Context Description Language (ACDL) schemas maps directly to a named Python function in the \texttt{doitlib/context.py} module:
\begin{itemize}
    \item \texttt{AGENT\_INSTRUCTIONS} $\longleftrightarrow$ \texttt{context.agent\_instructions()}
    \item \texttt{env.cwd}, \dots $\longleftrightarrow$ \texttt{context.environment\_block()}
    \item \texttt{env.history} $\longleftrightarrow$ \texttt{context.history\_messages()}
\end{itemize}
This precise mapping guarantees that the formal specifications reflect the actual runtime context injected into the LPU during each turn loop execution. Figure~\ref{fig:context_map} represents this context assembly process.
...
-rw-r--r--@  1 yuval  staff  12405 Jul  7 18:24 README.md
...
-rw-r--r--@ 1 yuval  staff   12K Jul  7 18:24 README.md
$ doit "which of these is larger?"
DECISIONS.md is the larger file, with a size of 30K compared to README.md's 12K.
\end{lstlisting}
