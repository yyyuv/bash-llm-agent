...
\begin{titlepage}
    ...
    \vfill
    {\large\color{secondary} August 2026 \par}
\end{titlepage}
...
Not every request maps to a command. A physically impossible request is declined with the \texttt{answer} tool, and an off-topic request receives a short in-role refusal rather than compliance --- in both cases nothing runs (\texttt{logs/phase1/p1demo.jsonl}):

\begin{lstlisting}[language=bash, caption={Impossible and off-topic requests: the model answers instead of running a command.}]
$ doit "make my laptop fly"
I'm a shell command agent and can't help with that!

$ doit "tell me a joke"
I'm a shell command agent and can't help with jokes, but I'm here to
assist you with commands and tasks!
\end{lstlisting}
...
To ensure the utility can run independently of commercial API keys, we extended the system to run models locally as well.
...
$ DOIT_SESSION=demo doit "by creation time"
The `ls` command on macOS does not support sorting by creation time
directly; it can only sort by modification time. Would you like to sort
by modification time instead?
\end{lstlisting}

The final turn shows the agent being honest about a platform limitation rather than parroting a flag: macOS/BSD \texttt{ls} has no portable creation-time sort, so it answers instead of emitting a command that would not do what was asked (\texttt{logs/phase4/live\_multiturn\_gpt4omini.txt}).
...
The ACDL diagram below adds the replayed conversation history to the context:
...
The LPU manipulates these facts via two specialized tools: \texttt{remember(fact)} and \texttt{forget(id)}. Within the controller's turn loop, memory modifications are treated as within-turn steps: when the agent issues a memory tool call, the controller executes the state change in \texttt{\_handle\_memory} and immediately re-enters the execution loop without decrementing the command step limit (the rationale is in the design decision below).
...
The ACDL diagram below adds the persistent-memory block to the context:
...
The assignment's headline case, ``summarize what I just did'', additionally shows the user/\texttt{doit} split: the summary is built only from the commands the user typed, while \texttt{doit}'s own invocations are filtered out (by \texttt{load\_recent\_user\_commands}) before the block ever reaches the model (\texttt{logs/phase7/live\_summarize\_gpt4omini.txt}):

\begin{lstlisting}[language=bash, caption={Summarizing manual actions; prior doit invocations are excluded, not counted as user activity.}]
$ mkdir data2
$ cd logs
$ cd ..
$ mkdir data3
$ doit "summarize what I just did"
You created two directories named data2 and data3 in your current project
folder, and navigated into the logs directory and back. (The earlier
doit "..." calls are excluded -- they are the agent's own actions, not yours.)
\end{lstlisting}
...
The ACDL diagram below adds the manual user shell-history block to the context:
...
When building the context for subsequent turns, the controller replays the past turns, applying the same hot/cold head-and-tail truncation described in Section~\ref{sec:multi_turn} (\texttt{truncate\_for\_context} in \texttt{doitlib/tools.py}): the current turn keeps the larger hot budget so a follow-up can still see its full output, while older turns are cut to the smaller cold budget to save tokens.
...
The ACDL diagram below adds the cross-session summaries and the \texttt{read\_session} retrieval tool to the context:
...
The \texttt{doit} agent implements every section of the assignment on top of a single architectural rule: the Controller owns all state, safety, and execution, and the LPU only ever emits a structured tool decision. Holding that boundary is what let each later feature enter as exactly one of three things --- a new tool, a new context block, or new controller logic --- without disturbing the parts already built, and it let the same eight-tool schema drive three different models through one interface.

The measurements support the design's two safety nets. The deterministic guard overrode the model's own \texttt{is\_destructive} flag in 6 of 15 cases, and the prompted-adapter parser recovered malformed JSON in 10 of 10 offline cases, while the native adapter produced no tool-call failures across the live comparison (Section~\ref{sec:model_flexibility}). Model competence, however, was not uniform: on the single-command suite \texttt{gpt-4o-mini} scored 10/13 against 7/13 and 6/13 for the 4B local models, and the weaker models exposed failure modes the framework cannot fix from outside --- over-flagging read-only pipelines as destructive, emitting GNU-only flags on BSD/macOS, and parroting their own replayed history.

Several limitations remain, documented in their sections: history is a fixed $K=10$ window with no compaction, memory IDs are recycled, the regex guard trades precision for determinism, and a plan budget can be exhausted by a model that does not batch same-type actions. The most useful lessons came from the failures we hit and fixed live --- a prompt rule that only worked once it was moved to the point of conflict, a memory edit that crashed on parallel tool-calls, and a plan that stopped halfway --- and those, more than the passing cases, shaped the final system.
...
The LPU emits exactly one tool call per step. The eight tools and their required arguments are listed in Table~\ref{tab:tools_overview} (Section~\ref{sec:tool_set}); their complete definitions in \texttt{doitlib/tools.py} follow below. For the native adapter these travel out-of-band via LiteLLM's \texttt{tools=} parameter; for the prompted adapter they are rendered into the system prompt and the model must reply with a single JSON object of the shape \texttt{\{"tool": "<name>", "args": \{ ... \}\}}.
