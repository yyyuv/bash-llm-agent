...
\subsection{System Prompt (base)}
Every turn starts with this single \texttt{system}-role message, returned by \texttt{agent\_instructions()} (\texttt{prompts/system\_prompt.txt}). All the per-feature guidance (safety, clarify, memory, change\_dir, cross-session, plan) lives in this one file --- this is the ``one system message'' our ACDL specs draw as separate \texttt{*\_INSTRUCTIONS} blocks.
\lstinputlisting[numbers=none, basicstyle=\ttfamily\scriptsize, breaklines=true]{../prompts/system_prompt.txt}

\subsection{Prompted-Adapter Additions}
For local models without native tool-calling, the tool schemas (as text) plus this JSON-only wrapper are appended to the system prompt (\texttt{prompts/prompted\_suffix.txt}); the \texttt{\{tools\}} placeholder is filled with the signatures from Section~\ref{sec:tool_schemas}.
