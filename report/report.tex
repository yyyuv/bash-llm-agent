...
Following the core architecture of LLM systems, we establish a strict boundary between the \textbf{Language Processing Unit (LPU)}—responsible solely for generating structured tool decisions from context—and the \textbf{Controller}—a deterministic Python runtime environment that manages state, enforces safety checks, and handles command execution.
...
\begin{decisionbox}{Design Decision: Shell-Function Wrapper for Directory Navigation (\texttt{cd})}
    ...
    \textbf{Dual-Shell Snippet (Zsh / Bash):}
    \begin{lstlisting}[language=bash]
doit() {
  command doit "$@"
  local target=~/.doit/cd_target_$DOIT_SESSION
  [ -f "$target" ] && cd "$(cat "$target")" && rm "$target"
}
    \end{lstlisting}
\end{decisionbox}
...
\begin{decisionbox}{Design Decision: Dual-Shell Variable Normalization}
    ...
    \textbf{Solution:} We normalize shell interactions at the boundary. The Controller auto-detects the host shell via \texttt{\$SHELL} and includes the shell type and operating system (BSD vs. GNU) in the environment prompt block. The system prompt instructs the LPU to emit shell-agnostic commands or syntax tailored to the host environment.
\end{decisionbox}
...
\begin{decisionbox}{Design Decision: In-Band Self-Report vs. Separate LLM Judge Call}
    \textbf{Alternative Rejected:} We rejected using a separate LLM call to classify command destructiveness (as suggested in the assignment description). 
...
\subsection{ACDL Context Specification: DoitAgentV2}
...
\begin{figure}[H]
    ...
    \caption{Visual representation of the ACDL context for DoitAgentV2 (Safety Gate).}
    \label{fig:acdl_v2}
\end{figure}

\subsection{Limitations \& Imperfect Behavior}
\begin{itemize}
    \item \textbf{Interactive SSH False Negatives:} In \texttt{safety.py}, we identify interactive SSH sessions by checking if the command contains more than one non-flag token. A command such as \texttt{ssh -p 2222 host} is parsed as having multiple arguments and passes as non-interactive, hanging the subprocess execution when it drops into a terminal.
    \item \textbf{Regex False Positives:} Commands such as \texttt{grep "rm -rf" notes.txt} contain write patterns inside text patterns. The regex guard overrides this as destructive, forcing a redundant \texttt{y/N} confirmation on a read-only command.
\end{itemize}
...
\subsection{ACDL Context Specification: DoitAgentV3Prompted}
...
\begin{figure}[H]
    ...
    \caption{Visual representation of the ACDL context for DoitAgentV3Prompted (Prompted Adapter).}
    \label{fig:acdl_v3}
\end{figure}
...
\subsection{Limitations \& Divergences}
Under local instruction models (such as \texttt{gemma3:4b}), we observed a failure where the model parroted the same command (\texttt{ls -la | sort -k 5 -r}) across multiple turns instead of adjusting the sort arguments. This indicates that small local models struggle with context tracking and copy previous outputs rather than adjusting parameters.
...
\begin{lstlisting}[language=bash, caption={Clarification question menu and user choice.}]
$ DOIT_SESSION=demo doit "delete the logs"
...
\subsection{ACDL Context Specification: DoitAgentV5Clarify}
...
\begin{figure}[H]
    ...
    \caption{Visual representation of the ACDL context for DoitAgentV5Clarify (Clarifications \& Multi-turn).}
    \label{fig:acdl_v5}
\end{figure}
...
% SECTION 8: ENVIRONMENT FRICTION LOG
\section{System Environment \& Friction Log}
\label{sec:friction_log}
Developing an agent that executes commands directly in a system shell exposes several environment-specific failure modes. We cataloged these incidents below:

\begin{enumerate}
    \item \textbf{Conda vs. System Python Conflicts:} The host machine had active Conda and system Python installations. Sourcing the Conda environment put Conda's Python interpreter first on the \texttt{PATH}, which did not have \texttt{litellm} installed. This caused a \texttt{ModuleNotFoundError} when calling \texttt{doit}. Sourcing \texttt{conda deactivate} resolved the conflict.
    
    \item \textbf{Shell Sourcing vs. Exporting:} Sourcing the \texttt{.env} file using \texttt{source .env} defined \texttt{OPENAI\_API\_KEY} as a local shell variable, but parent processes could not read it. Sourcing must explicitly export variables:
    \begin{lstlisting}[language=bash]
    set -a; source .env; set +a
    \end{lstlisting}
    
    \item \textbf{PATH Corruption:} A manual edit in \texttt{\textasciitilde/.zshrc} corrupted the \texttt{PATH} syntax, resulting in a \texttt{zsh: command not found: doit} error. The PATH variable was restored to its clean declaration: \texttt{export PATH="\$HOME/.local/bin:\$PATH"}.
\end{enumerate}

% SECTION 9: CONCLUSION
\section{Conclusion \& Limitations Summary}
We successfully built the \texttt{doit} agentic shell in accordance with a clean Controller-LPU separation. The agent supports safety gates, native/prompted adapters, multi-turn history with tiered budgets, and within-turn clarification loops. 

Our main open limitation is safety regex false positives (e.g. flagging \texttt{grep "rm -rf"} as destructive) and parsing interactive options (e.g. \texttt{ssh -p 2222}) where the regex check does not capture interactive states. However, the system is secure and successfully delegates complex execution logic to local and remote language models.
