...
**Phase 9 — extension (1 day). ✅ DECIDED (D11, 2026-07-08): implement #1 multi-step command plans WITH self-correcting retry folded in; fallback to retry-alone if plans prove too hard on the native model.** Chosen because we *felt the need live*: Phase 8's window-2 case ("create a folder for each year from 2020 to 2026") silently completed 1/7 of the task in single-command mode (`mkdir 2020`, turn over) until the request was re-phrased "in one command" — silent partial completion is exactly what the step cap costs, and the loop already proved it wants multi-step (`read_session` → act, live). Describe three in the report:
   1. **Multi-step command plans + self-correcting retry** *(implemented)* — a new opt-in `plan(steps[])` tool: controller prints the plan, raises THIS turn's command budget (default stays single-command — no global `max_steps` bump), per-step destructive `y/N` gates retained, each output feeds the next step; retry = on any `rc≠0` the model gets stderr back and one corrective attempt (always on, all models). Capability-gated by `enable_plans` in `doit.cfg` per MODEL (on for gpt-4o-mini, off for the 4B locals — native or prompted alike; gate on model capability, not adapter, so adapter-interchangeability stays intact; weak models still get retry). Known cons, stated upfront (report material): cost/latency ×steps; context growth per step (truncation can starve later steps); compounding errors from a misread early step; a NEW failure mode — half-executed plans (a declined/failed middle step leaves partial state single-command mode could never create); the printed plan is not binding (the model may deviate from what the user confirmed); weak models will produce incoherent sequences (capture ONE such failure transcript deliberately — comparison-chapter content).
   2. Undo journal — `doit "undo that"`: record inverse commands for invertible destructive actions (`mkdir X`→`rmdir X`, `mv A B`→`mv B A`), honestly refuse when not invertible (`rm` is gone).
   3. Dry-run preview — destructive commands first run a read-only preview variant showing the blast radius (`rm *.tmp` → first `ls *.tmp`) before the `y/N` gate.
   (Context compaction and `.doit.md` project profiles remain documented-considered alternates, superseded as the described trio.)
